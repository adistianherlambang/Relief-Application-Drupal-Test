...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for theme695
 */

// Auto-rebuild the theme registry during theme development.
if (theme_get_setting('theme695_clear_registry')) {
  // Rebuild .info data.
 ...
 */
function theme695_html_head_alter(&$head_elements) {
 ...
 */
function theme695_preprocess_search_block_form(&$vars) {
 ...
 */
function theme695_preprocess(&$vars, $hook) {
  $vars['theme695_path'] = base_path() . path_to_theme();
}

/**
 * Implements template_preprocess_html().
 */
function theme695_preprocess_html(&$vars) {
  
  $vars['doctype'] = _theme695_doctype();
  $vars['rdf'] = _theme695_rdf($vars);
  ...
  if (!$vars['is_front']) {
    ...
    if (isset($page_name)) {
      $vars['classes_array'][] = theme695_id_safe('page-' . $page_name);
    }

    $vars['classes_array'][] = theme695_id_safe('section-' . $section);
 ...
 */
function theme695_preprocess_page(&$vars) {
 ...
 */
function theme695_preprocess_maintenance_page(&$vars) {
  ...
  drupal_add_css(drupal_get_path('theme', 'theme695') . '/css/maintenance-page.css');
}
 ...
 */
function theme695_preprocess_node(&$vars) {
  // Striping class
 ...
 */
function theme695_preprocess_block(&$vars, $hook) {
 ...
 */
function theme695_menu_tree($vars) {
 ...
 */
function theme695_field__taxonomy_term_reference($vars) {
  $output = '';
 ...
 */
function theme695_breadcrumb($vars) {
  
  $breadcrumb = isset($vars['breadcrumb']) ? $vars['breadcrumb'] : array();

// Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('theme695_breadcrumb_display');
  if ($show_breadcrumb == 'yes') {
  
	  if (theme_get_setting('theme695_breadcrumb_hideonlyfront')) {
		$condition = count($breadcrumb) > 1;
	  } else {
		$condition = !empty($breadcrumb);
	  }
	  
	  if(theme_get_setting('theme695_breadcrumb_showtitle')) {
	  ...
	  $separator = theme_get_setting('theme695_breadcrumb_separator');
 ...
 * @return bool
 */
function theme695_tabs_float() {
  $float = (bool) theme_get_setting('theme695_tabs_float');
  $float_node = (bool) theme_get_setting('theme695_tabs_node');
...
function theme695_id_safe($string) {
  // Strip accents
 ...
 * @ingroup themeable
 */
function theme695_menu_link(array $vars) {
  ...
  $element['#attributes']['class'][] = theme695_id_safe($element['#title']);
 ...
 */
function theme695_preprocess_menu_local_task(&$vars) {
 ...
 */
function theme695_menu_local_tasks(&$vars) {
  $output = '';
